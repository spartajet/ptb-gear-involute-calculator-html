...
class InputCheckAngle(val secondAllowDigits: Int, val checkLimitMax: Angle, val checkLimitMin: Angle, val result: InputCheckResultAngle = InputCheckResultAngle()) : InputCheck(result) {
    ...
    override fun addChar(c: Char): InputCheckResult {
        ...
        if (isbackSpace) {
            ...
            when (this.cursor) {
                ...
                Cursor.CURSOR_ANGLE_DEGREE_INTEGER -> this.angleDegreeIntegerCheck(c)
                Cursor.CURSOR_ANGLE_DEGREE_SIGNAL, Cursor.CURSOR_ANGLE_MINUTS_INTEGER -> this.angleMinuteIntegerCheck(c)
                Cursor.CURSOR_ANGLE_MINUTS_SIGNAL, Cursor.CURSOR_ANGLE_SECOND_INTEGER -> this.angleSecondIntegerCheck(c)
                Cursor.CURSOR_ANGLE_SECONDE_DOT, Cursor.CURSOR_ANGLE_SECOND_DECIMAL -> this.angleSecondDecimalCheck(c)
                Cursor.CURSOR_ANGLE_SECOND_SIGNAL -> {
    ...
    private fun backSpaceCheck() {
        when (cursor) {
            ...
            Cursor.CURSOR_ANGLE_DEGREE_INTEGER -> {
                this.degreeList.removeAt(this.degreeList.size - 1)
                if (degreeList.size == 0) {
                    cursor = Cursor.CURSOR_START
                }
            }
            Cursor.CURSOR_ANGLE_DEGREE_SIGNAL -> this.cursor = Cursor.CURSOR_ANGLE_DEGREE_INTEGER
            Cursor.CURSOR_ANGLE_MINUTS_INTEGER -> {
                this.minuteList.removeAt(this.minuteList.size - 1)
                if (minuteList.size == 0) {
                    cursor = Cursor.CURSOR_ANGLE_DEGREE_SIGNAL
                }
            }
            Cursor.CURSOR_ANGLE_MINUTS_SIGNAL -> this.cursor = Cursor.CURSOR_ANGLE_MINUTS_INTEGER
            Cursor.CURSOR_ANGLE_SECOND_INTEGER -> {
            ...
            Cursor.CURSOR_ANGLE_SECONDE_DOT -> this.cursor = Cursor.CURSOR_ANGLE_SECOND_INTEGER
            Cursor.CURSOR_ANGLE_SECOND_DECIMAL -> {
            ...
            Cursor.CURSOR_ANGLE_SECOND_SIGNAL -> this.cursor = Cursor.CURSOR_ANGLE_SECOND_DECIMAL
        }
    }
    ...
    private fun generateResultValue() {
        ...
        var secondDecimal = 0
        when (cursor) {
            Cursor.CURSOR_START -> this.value = 0.0
            Cursor.CURSOR_ANGLE_DEGREE_INTEGER, Cursor.CURSOR_ANGLE_DEGREE_SIGNAL -> this.value = this.getIntegerFromList(this.degreeList).toDouble()
            Cursor.CURSOR_ANGLE_MINUTS_INTEGER, Cursor.CURSOR_ANGLE_MINUTS_SIGNAL -> {
            ...
            Cursor.CURSOR_ANGLE_SECOND_INTEGER, Cursor.CURSOR_ANGLE_SECONDE_DOT -> {
                degree = this.getIntegerFromList(this.degreeList)
                minute = this.getIntegerFromList(this.minuteList)
                secondInteger = this.getIntegerFromList(this.secondIntegerList)
                this.value = degree.toDouble() + minute / 60.0 + secondInteger / 3600.0
            }
            Cursor.CURSOR_ANGLE_SECOND_DECIMAL, Cursor.CURSOR_ANGLE_SECOND_SIGNAL -> {
                degree = this.getIntegerFromList(this.degreeList)
                minute = this.getIntegerFromList(this.minuteList)
                secondInteger = this.getIntegerFromList(this.secondIntegerList)
    ...
    private fun generateResultValueString() {
        ...
        when (this.cursor) {
            Cursor.CURSOR_START -> this.valueString = ""
            Cursor.CURSOR_ANGLE_DEGREE_INTEGER -> this.degreeList.forEach { builder.append(it) }
            Cursor.CURSOR_ANGLE_DEGREE_SIGNAL -> {
            ...
            Cursor.CURSOR_ANGLE_MINUTS_SIGNAL -> {
                this.degreeList.forEach { builder.append(it) }
                builder.append(ANGLE_DEGREE_SIGNAL)
                this.minuteList.forEach { builder.append(it) }
            ...
            Cursor.CURSOR_ANGLE_SECONDE_DOT -> {
                this.degreeList.forEach { builder.append(it) }
                builder.append(ANGLE_DEGREE_SIGNAL)
                this.minuteList.forEach { builder.append(it) }
                builder.append(ANGLE_MINUTE_SIGNAL)
